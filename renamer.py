# Sequential file renamer with filtering, safety, and nested depth support
"""
                        SEQUENTIAL FILE RENAMER
                        -----------------------
This script renames files in the current directory or nested subdirectories sequentially.
It supports custom prefixes, suffixes, and sorting methods while
retaining media filters, safety features, and full undo capabilities.
...
USAGE:
        ...
        python renamer.py -sd -p "Sub_"            # Renames files within each subfolder sequentially (depth 1)
        python renamer.py -sd "folder*" -p "Sub_"  # Renames files inside subfolders starting with "folder"
        python renamer.py --depth 2 -t image       # Renames images in subdirectories 2 levels deep (e.g. /sd1/sub1/)
        python renamer.py --depth 3 -f -p "IMG_"   # Renames files in folders 3 levels deep using folder name prefix
        python renamer.py -t folder -p "DIR_"      # Renames directories instead of files

OPTIONS:
        -p, --prefix STR     Text to put before the content
        -f, --folder         Use the parent/subfolder name as the prefix
        ...
        --depth N            Process folders located at an exact depth N from current folder (N >= 1).
                             For example, --depth 2 processes all /sub1/sub2/ directories.
...
def perform_undo():
    ...
    for old_path, new_path in reversed(history):
        if not os.path.exists(new_path):
            print(
                f"{CLR_RED}Missing: {new_path}{CLR_RESET} (cannot restore to {os.path.basename(old_path)})"
            )
            continue
        ...
        try:
            ...
            print(
                f"Restored: {CLR_ORANGE}{os.path.basename(new_path)}{CLR_RESET} -> {CLR_GREEN}{os.path.basename(old_path)}{CLR_RESET}"
            )
        except Exception as e:
...
def get_directories_at_depth(base_dir, target_depth, pattern=None):
    """
    Recursively finds all subdirectories located exactly at `target_depth` levels
    beneath `base_dir`. Depth 1 refers to immediate subdirectories.
    """
    results = []

    def scan_depth(current_path, current_depth):
        ...
        try:
            ...
            for subdir in subdirs:
                if current_depth == target_depth:
                    if pattern is None or fnmatch.fnmatch(subdir.name, pattern):
                        results.append(subdir.path)
                else:
                    scan_depth(subdir.path, current_depth + 1)
        except OSError as e:
            sys.stderr.write(
                f"{CLR_RED}Error scanning {current_path}: {e}{CLR_RESET}\n"
            )

    scan_depth(base_dir, 1)
    results.sort()
    return results
            ...
            for entry in entries:
                if target_type == "folder":
                    if (
                        ...
                        # Wildcard Filter
                        if wildcard_pattern and not fnmatch.fnmatch(
                            filename, wildcard_pattern
                        ):
                            continue
...
def rename_files(
    ...
    folder_display = (
        rel_folder
        if rel_folder != "."
        else os.path.basename(os.path.abspath(folder_path))
    )

    if not files:
        print(f"No matching items found in '{folder_display}'. Nothing to do.")
        return history
    ...
    if striponly_str is None:
        if digit_count is None:
            ...
            print(f"[{folder_display}] Auto-calculated digit padding: {digit_count}")
        elif digit_count > 0:
            if digit_count < required_digits:
                sys.stderr.write(
                    f"{CLR_RED}Error: Specified digits ({digit_count}) is too small for {num_files} files in '{folder_display}'.{CLR_RESET}\n"
                )
            ...
            if is_folder_mode:
                print(
                    f"[{folder_display}] Numbering disabled. Using original folder names."
                )
            else:
                print(
                    f"[{folder_display}] Numbering disabled. Using original filenames."
                )
    else:
        if is_folder_mode:
            print(
                f"[{folder_display}] Strip-only mode: Removing '{striponly_str}' from folder names."
            )
        else:
            print(
                f"[{folder_display}] Strip-only mode: Removing '{striponly_str}' from filenames."
            )

    item_type_str = "folders" if is_folder_mode else "files"
    print(f"Processing {num_files} {item_type_str} in '{folder_display}'...\n")
    ...
    for i, filename in enumerate(files, start=1):
        ...
        try:
            if keep:
                ...
                print(
                    f"Copied: {CLR_ORANGE}{filename}{CLR_RESET} -> {display_color}{final_name}{CLR_RESET}"
                )
            else:
                os.rename(old_path, new_path)
                print(
                    f"Renamed: {CLR_ORANGE}{filename}{CLR_RESET} -> {display_color}{final_name}{CLR_RESET}"
                )
    ...
    print()  # Spacer between folders
    return history


if __name__ == "__main__":
    parser = ColoredArgumentParser(
        description="Sequentially rename files in the current folder or subdirectories."
    )
    ...
    rename_opts.add_argument(
        "-p", "--prefix", type=str, default="", help="Prefix for the new filename."
    )
    rename_opts.add_argument(
        "-f",
        "--folder",
        action="store_true",
        help="Use the current folder name as prefix (or the matching subfolder name if using -sd or --depth).",
    )
    rename_opts.add_argument(
        "-s",
        "--suffix",
    ...
    rename_opts.add_argument(
        ...
        default="name_asc",
        help="Sorting method (default: name_asc)",
    )
    rename_opts.add_argument(
        "-k",
        "--keep",
    ...
    rename_opts.add_argument(
        ...
        "--type",
        type=str,
        choices=["image", "video", "audio", "folder"],
        help="Filter by category: 'image', 'video', 'audio', or 'folder'",
    )
    rename_opts.add_argument(
        "-e", "--ext", type=str, help="Filter for a specific extension only."
    )
    rename_opts.add_argument(
        "-a",
        "--all",
    ...
    rename_opts.add_argument(
        ...
        const="*",
        type=str,
        help="Process subfolders individually. Can optionally target a specific pattern (e.g. -sd 'folder*').",
    )
    rename_opts.add_argument(
        "--depth",
        type=int,
        default=None,
        help="Target subdirectories at a specific nesting depth (1 = immediate subfolders, 2 = sub-subfolders, etc.).",
    )
    ...
    if args.undo:
        for arg in sys.argv[1:]:
            if arg in [
                ...
                parser.error(
                    "The --undo flag is exclusive and cannot be used with other options."
                )
        perform_undo()
        sys.exit(0)
    ...
    if args.depth is not None and args.depth < 1:
        parser.error("--depth must be an integer greater than or equal to 1.")
    ...
    if args.striponly:
        if any(
            ...
            parser.error(
                "--striponly (-xx) can only be combined with -w, -t, -a, -e, -k, -sd, and --depth."
            )
    ...
    if args.all:
        confirm = input(
            f"{CLR_RED}WARNING: You are using the --all flag. This will process system files and scripts.{CLR_RESET}\nAre you sure? (y/n): "
        )
        if confirm.lower() != "y":
    ...
    # Check if depth or subdir processing is active
    is_depth_mode = args.depth is not None
    is_subdir_mode = args.subdir is not None

    if is_depth_mode or is_subdir_mode:
        effective_depth = args.depth if is_depth_mode else 1
        pattern = args.subdir if is_subdir_mode else None

        subfolders = get_directories_at_depth(
            current_folder, effective_depth, pattern=pattern
        )

        if not subfolders:
            pattern_desc = f" matching '{pattern}'" if pattern else ""
            print(
                f"No subdirectories found at depth {effective_depth}{pattern_desc}. Nothing to do."
            )
            sys.exit(0)

        # Run sequential renaming in each target subfolder individually
        for subfolder_path in subfolders:
            ...
            target_files = get_target_files(
                subfolder_path, allowed, args.all, args.wildcard, target_type=args.type
            )
            if args.striponly:
        ...
        # Standard single-folder processing (working directory)
        final_prefix = args.prefix
        if args.folder:
            final_prefix = os.path.basename(current_folder)

        target_files = get_target_files(
            current_folder, allowed, args.all, args.wildcard, target_type=args.type
        )
        if args.striponly:
            target_files = [f for f in target_files if args.striponly in f]
