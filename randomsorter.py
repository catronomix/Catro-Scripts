...
def distribute_images(folder_path, counts, allowed_extensions):
	...
	"""
	# First, perform the random renaming. If no images found, stop early.
	if not rename_images_in_folder(folder_path, allowed_extensions):
		print(f"No images found with extensions {allowed_extensions}. Nothing to do.")
		return
	...
	for i, count in enumerate(counts):
		if not images:
			print("\nNo more images left to distribute.")
			break

		subfolder_name = f"reeks{i + 1}"
		subfolder_path = os.path.join(folder_path, subfolder_name)
		
		# Only create the directory and start moving if we have images left
		os.makedirs(subfolder_path, exist_ok=True)
		print(f"\nPopulating {subfolder_name} with up to {count} images...")
